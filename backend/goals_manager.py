...
class GoalsManager:
    ...
    def calculate_goal_progress(self, goal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Handle created_at - if missing (legacy), assume 1 year before target
        if 'created_at' in goal:
            created_at = datetime.fromisoformat(goal['created_at'].replace('Z', '+00:00'))
        else:
            created_at = target_date - timedelta(days=365)
            
        now = datetime.utcnow().replace(tzinfo=target_date.tzinfo)
        ...
        if total_duration > 0:
            ...
            expected_progress = max(0, min(100, expected_progress)) # Clamp between 0 and 100
        else:
            expected_progress = 100
            
        # Check if on track (allow 10% deviation)
        on_track = progress_percentage >= (expected_progress * 0.9)
