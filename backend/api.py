...
visualization_service = VisualizationService(mcp_client)
...
class VisualizeRequest(BaseModel):
    query: str = Field(..., description="User query for visualization")
    chart_type: Optional[str] = Field(None, description="Specific chart type requested")
...
async def visualize_data(
    ...
    """Generate data for visualization based on user query."""
    try:
        ...
        if any(word in query for word in ['trend', 'over time', 'history', 'spending']):
            ...
            if "last year" in query or "1 year" in query:
                period = "last_year"
            elif "last 3 months" in query or "3 months" in query:
...
@router.get("/api/quick-insights")
async def get_quick_insights(sessionid: str = Depends(get_sessionid)):
    """Get quick financial insights for the dashboard."""
    try:
        # Fetch necessary data in parallel
        net_worth_data, bank_data, mf_data, stock_data = await asyncio.gather(
            mcp_client.get_net_worth(sessionid),
            mcp_client.get_bank_transactions(sessionid),
            mcp_client.get_mf_transactions(sessionid),
            mcp_client.get_stock_transactions(sessionid)
        )
        ...
        for txn in transactions:
            ...
            if txn_date and last_month <= txn_date <= today and (txn.get('txn_type') == 'DEBIT' or txn.get('type') == 'expense'):
                monthly_spend += txn.get('amount', 0)

        # Get top investment
        investments = [txn for txn in transactions if txn.get('category') == 'Investment' and txn.get('amount') is not None]
        top_investment = max(investments, key=lambda x: x.get('amount', 0)) if investments else None
            ...
            "insights": [
                {
                    "type": "net_worth",
                    "title": "Net Worth",
                    "value": f"₹{int(net_worth_data['netWorthResponse']['totalNetWorthValue']['units']):,}",
                ...
                },
                {
                    ...
                    "value": top_investment['narration'] if top_investment else "N/A",
                    "subtitle": f"Value: ₹{top_investment['amount']:,}" if top_investment else ""
                }
            ],
            "quick_actions": [
                {
                    "label": "Analyze my recent spending",
                    "query": "Analyze my spending for the last 30 days"
                },
                {
                    "label": "How can I improve my investments?",
                    "query": "How can I improve my investment portfolio?"
                }
            ]
        }
        return insights
    except Exception as e:
        logging.error(f"Error in get_quick_insights: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_transaction_summary_internal(from_date: str, to_date: str, sessionid: str) -> Dict[str, Any]:
    """
    Internal function to get transaction summary for a date range.
    """
    # Fetch all transactions
    bank_data = await mcp_client.get_bank_transactions(sessionid)
    mf_data = await mcp_client.get_mf_transactions(sessionid)
...
async def quick_insights(request: Request):
    ...
    logging.info(f"Context for quick_insights: {context}")
    
    insights = []
    
    # 1. Net Worth
    if 'net_worth' in context and 'netWorthResponse' in context['net_worth']:
        net_worth_data = context['net_worth']
        total = net_worth_data['netWorthResponse'].get('totalNetWorthValue', {})
        if total and total.get('units'):
    ...
    # 3. Next Payment (assuming upcoming_payments is a list of dicts)
    if 'upcoming_payments' in context and context['upcoming_payments']:
        next_payment = context['upcoming_payments'][0]
        if next_payment.get('category') and next_payment.get('amount') and next_payment.get('due'):
            amount = int(next_payment['amount'])
            insights.append({
                "type": "next_payment",
                "title": "Next Payment",
                "value": f"{next_payment['category']} - ₹{amount:,}",
    ...
    if 'spending_summary' in context:
        spend_data = context['spending_summary']
        top_cats = spend_data.get('top_categories', [])
        if top_cats:
            top = top_cats[0]
            if top.get('category') and top.get('percentage'):
                insights.append({
                    "type": "top_category",
                    "title": "Top Spending",
                    "value": top['category'],
    ...
    # 5. Credit Score
    if 'credit_report' in context and 'creditReports' in context['credit_report']:
        credit_data = context['credit_report']
        if credit_data['creditReports'] and 'creditReportData' in credit_data['creditReports'][0]:
            report = credit_data['creditReports'][0]['creditReportData']
            if 'score' in report and report['score'].get('bureauScore'):
                score = report['score'].get('bureauScore')
    ...
    if 'goals' in context and context['goals']:
        # Assuming goals is a list of dictionaries
        # Find the goal with the highest progress percentage
        closest_goal = None
        max_progress = -1
        for goal in context['goals']:
            if goal.get('progress_percentage', 0) > max_progress:
                max_progress = goal['progress_percentage']
                closest_goal = goal
        
        if closest_goal and closest_goal.get('name') and closest_goal.get('progress_percentage') is not None:
            current = int(closest_goal.get('current_amount', 0))
            target = int(closest_goal.get('target_amount', 0))
            insights.append({
                "type": "goal_progress",
                "title": closest_goal['name'],
                "value": f"{closest_goal['progress_percentage']}% complete",
                "subtitle": f"₹{current:,} of ₹{target:,}",
                "icon": "🎯"
            })
