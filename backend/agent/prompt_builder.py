...
def build_system_prompt() -> str:
    ...
    if template_path.exists():
        ...
        base_prompt = """You are Prospera, a personal-finance AI assistant. 
...
def build_prompt(user_prompt: str, context: dict) -> str:
    ...
    lines.append("- Identify any anaomalies or unusual patterns")
...
async def build_enhanced_context(sessionid: str, mcp_client, goals_manager=None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Fetched net_worth: {context['net_worth'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching net_worth in build_enhanced_context: {e}")
        pass
    
    try:
        context['credit_report'] = await mcp_client.get_credit_report(sessionid)
        logger.info(f"Fetched credit_report: {context['credit_report'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching credit_report in build_enhanced_context: {e}")
        pass
    
    try:
        context['epf_details'] = await mcp_client.get_epf_details(sessionid)
        logger.info(f"Fetched epf_details: {context['epf_details'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching epf_details in build_enhanced_context: {e}")
        pass
    
    try:
        context['mf_transactions'] = await mcp_client.get_mf_transactions(sessionid)
        logger.info(f"Fetched mf_transactions: {context['mf_transactions'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching mf_transactions in build_enhanced_context: {e}")
        pass
    
    try:
        bank_data_context = await mcp_client.get_bank_transactions(sessionid)
        context['bank_transactions'] = bank_data_context
        logger.info(f"Fetched bank_transactions: {context['bank_transactions'] is not None}")
        
        # The spending summary is now calculated within _fetch_bank_data in ai_assistant.py
        # We need to ensure that the context for quick_insights uses the same logic or gets the summary from there.
        # For now, let's directly use the TransactionProcessor with a wide date range for quick_insights
        transactions = TransactionProcessor.parse_bank_transactions(bank_data_context)
        
        if transactions:
            # Use a wide date range for quick insights spending summary
            start_date = datetime(2020, 1, 1)
            end_date = datetime(2025, 12, 31)
            
            monthly_spend_data = TransactionProcessor.calculate_monthly_spend(
                transactions,
                start_date.strftime('%Y-%m-%d'),
                end_date.strftime('%Y-%m-%d')
            )
            
            category_breakdown_data = TransactionProcessor.calculate_category_breakdown(
                transactions,
                start_date.strftime('%Y-%m-%d'),
                end_date.strftime('%Y-%m-%d')
            )
            
            # Calculate average monthly spend from the monthly_spend_data
            total_spend_amount = sum(m['amount'] for m in monthly_spend_data)
            num_months = len(monthly_spend_data)
            monthly_avg = total_spend_amount / num_months if num_months > 0 else 0

            context['spending_summary'] = {
                'monthly_avg': monthly_avg,
                'top_categories': category_breakdown_data.get('breakdown', [])[:5]
            }
            logger.info(f"Calculated spending_summary: {context['spending_summary'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching bank_transactions or calculating spending_summary in build_enhanced_context: {e}")
        pass
    
    try:
        context['stock_transactions'] = await mcp_client.get_stock_transactions(sessionid)
        logger.info(f"Fetched stock_transactions: {context['stock_transactions'] is not None}")
    except Exception as e:
        logger.error(f"Error fetching stock_transactions in build_enhanced_context: {e}")
        pass
    ...
    if goals_manager:
        try:
            ...
            if goals:
                ...
                logger.info(f"Fetched goals: {context['goals'] is not None}")
        except Exception as e:
            logger.error(f"Error fetching goals in build_enhanced_context: {e}")
            pass
